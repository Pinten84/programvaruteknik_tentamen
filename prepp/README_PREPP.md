# Förberedelsematerial för Muntlig Examination 📚

Välkommen! Denna mapp innehåller allt du behöver för att förbereda dig för den muntliga examinationen.

---

## 📁 Innehåll

### 🌟 NYA DOKUMENT (För Muntlig Examination)

#### 1. **LASKLAR_DETTA_FORST.md** ⭐ START HÄR!
- **Vad:** Översikt och studieplan
- **Tid:** 5 minuter
- **Syfte:** Guidar dig genom hur du använder materialet
- **Innehåll:**
  - Översikt av alla dokument
  - Rekommenderad studieplan (3 dagar)
  - Checklista: Är du redo?
  - Top 10 viktigaste koncept
  - Tips för olika lärstilar

#### 2. **muntlig_examination_guide.md** 📖 HUVUDDOKUMENT
- **Vad:** Omfattande guide som förklarar allt i enkla termer
- **Tid:** 45 minuter
- **Syfte:** Lära dig alla koncept från grunden
- **Innehåll:**
  - Vad är den muntliga examinationen?
  - SOLID-principer förklarat för lekmän
  - Designmönster förklarat för lekmän
  - Din arkitektur förklarat enkelt
  - 10+ troliga frågor med modellsvar
  - Vanliga fallgropar att undvika
  - Snabbreferens

#### 3. **ovningsfragar_med_svar.md** 💪 ÖVNINGSMATERIAL
- **Vad:** 30+ övningsfrågor med detaljerade modellsvar
- **Tid:** 60+ minuter (öva flera gånger)
- **Syfte:** Aktiv övning för att förbereda dig
- **Innehåll:**
  - Del 1: Grundläggande Förståelse (3 frågor)
  - Del 2: SOLID-principer (3 frågor)
  - Del 3: Designmönster (3 frågor)
  - Del 4: Kvalitetsattribut (3 frågor)
  - Del 5: Buggar och Förbättringar (2 frågor)
  - Del 6: Arkitektur och Design (3 frågor)
  - Del 7: Framtida Utökningar (2 frågor)
  - Del 8: Jämförelser (2 frågor)
  - Del 9: Tekniska Detaljer (3 frågor)
  - Del 10: Reflektionsfrågor (2 frågor)

#### 4. **visuell_guide.md** 📊 VISUELLT STÖD
- **Vad:** Diagram och visuella representationer
- **Tid:** 20 minuter
- **Syfte:** Visuellt stöd för att förstå och förklara
- **Innehåll:**
  - Arkitektur-översikt (diagram)
  - Jämförelse: Original vs Refaktorerad
  - SOLID-principer visualiserade
  - Designmönster visualiserade
  - Kvalitetsattribut visualiserade
  - Buggar visualiserade
  - Koppling och kohesion diagram
  - Snabb checklista

---

### 📝 BEFINTLIGA DOKUMENT (Från tidigare)

#### 5. **prep.md**
- Tidigare förberedelsematerial
- Kan användas som komplement

#### 6. **prep_kod.md**
- Tidigare kodförberedelse
- Kan användas som komplement

---

## 🎯 Snabbstart

### Om du har 3 dagar:
1. **Dag 1:** Läs LASKLAR_DETTA_FORST.md → muntlig_examination_guide.md → visuell_guide.md
2. **Dag 2:** Öva med ovningsfragar_med_svar.md (alla 30+ frågor)
3. **Dag 3:** Repetera svaga områden, simulera examination

### Om du har 1 dag:
1. **Morgon:** Läs muntlig_examination_guide.md (fokus på SOLID och Designmönster)
2. **Eftermiddag:** Öva med ovningsfragar_med_svar.md (fokus på Del 1-5)
3. **Kväll:** Läs Snabbreferens och titta på diagram i visuell_guide.md

### Om du har 2 timmar:
1. Läs "Snabbreferens" i muntlig_examination_guide.md (10 min)
2. Titta på diagram i visuell_guide.md (15 min)
3. Öva på 10 frågor från ovningsfragar_med_svar.md (60 min)
4. Läs "Tips för Examinationen" (10 min)
5. Öppna din README.md och skumma igenom (5 min)

---

## 📊 Dokumentstatistik

| Dokument | Sidor | Ord | Läs-tid | Övnings-tid |
|----------|-------|-----|---------|-------------|
| LASKLAR_DETTA_FORST.md | ~8 | ~2,500 | 5 min | - |
| muntlig_examination_guide.md | ~25 | ~8,000 | 45 min | - |
| ovningsfragar_med_svar.md | ~30 | ~10,000 | 30 min | 60+ min |
| visuell_guide.md | ~20 | ~5,000 | 20 min | - |
| **TOTALT** | **~83** | **~25,500** | **100 min** | **60+ min** |

**Total förberedelsetid:** 2.5-4 timmar (beroende på hur mycket du övar)

---

## ✅ Checklista: Har du läst allt?

- [ ] LASKLAR_DETTA_FORST.md
- [ ] muntlig_examination_guide.md
- [ ] visuell_guide.md
- [ ] ovningsfragar_med_svar.md (minst en gång)
- [ ] Övat på minst 10 frågor HÖGT
- [ ] Tittat på din egen kod (Game.java, AnimalScoring.java, etc.)
- [ ] Kört `mvn test` och verifierat att alla tester passerar

---

## 🎓 Vad Examinatorn Letar Efter

### ✅ Bra Tecken
- Du kan förklara med egna ord
- Du kan ge konkreta exempel från din kod
- Du kan jämföra med originalkoden
- Du kan resonera om för- och nackdelar
- Du visar förståelse för VARFÖR, inte bara VAD

### ❌ Dåliga Tecken
- Du säger "AI gjorde det så jag vet inte..."
- Du memorerar definitioner utan förståelse
- Du kan inte peka på konkret kod
- Du kan inte förklara dina designval
- Du säger att allt är perfekt utan reflektion

---

## 💡 Viktigaste Koncepten (Top 5)

Om du bara har tid för det absolut viktigaste:

### 1. **Single Responsibility Principle**
En klass, en uppgift. Exempel: AnimalScoring gör bara djurpoäng.

### 2. **Strategy Pattern**
Utbytbara algoritmer. Exempel: ScoringStrategy med 5 implementationer.

### 3. **Testbarhet**
Original: omöjlig att testa. Din kod: 17 automatiska tester på sekunder.

### 4. **Paketstruktur**
Tre paket: model (data), scoring (affärslogik), game (koordinering).

### 5. **Buggar du Fixade**
- Djurpoäng: frequency == 2 → pairs = count / 2
- Samlingar: dubbla varje → summera först, dubbla sedan

---

## 🚀 Tips för Framgång

### Innan Examinationen
1. **Förbered dig väl** - använd materialet
2. **Öva aktivt** - svara högt på frågor
3. **Titta på koden** - bekanta dig med dina klasser
4. **Slappna av** - lita på din förberedelse

### Under Examinationen
1. **Var dig själv** - använd egna ord
2. **Ge exempel** - peka på konkret kod
3. **Resonera** - visa ditt tänkande
4. **Var ärlig** - säg om du inte vet

### Om du Fastnar
1. **Be om förtydligande** - "Kan du omformulera?"
2. **Tänk högt** - "Låt mig resonera..."
3. **Använd analogier** - "Det är som att..."
4. **Peka på kod** - "Kan jag visa i koden?"

---

## 📞 Kontakt och Support

Om du har frågor eller behöver förtydliganden:
- Läs om i huvuddokumentet
- Öva mer på övningsfrågorna
- Titta på diagram i visuell guide
- Öppna din kod och utforska

---

## 🎉 Slutord

Du har nu tillgång till över 25,000 ord av förberedelsematerial som täcker:
- ✅ Alla SOLID-principer
- ✅ Alla designmönster du använt
- ✅ Din arkitektur i detalj
- ✅ 30+ övningsfrågor med svar
- ✅ Visuella diagram och representationer
- ✅ Tips och strategier för examinationen

**Du är väl förberedd!**

Kom ihåg: Det viktigaste är inte att kunna alla facktermer, utan att kunna förklara ditt tänkande och dina designval.

---

## 🍀 LYCKA TILL!

**Du klarar detta!** 💪

Börja med att läsa **LASKLAR_DETTA_FORST.md** för att få en studieplan.

---

**Skapad:** 2025-12-02  
**Syfte:** Förberedelse för muntlig examination i Mjukvaruteknik  
**Omfattning:** 4 nya dokument, ~83 sidor, ~25,500 ord  
**Förberedelsetid:** 2.5-4 timmar
